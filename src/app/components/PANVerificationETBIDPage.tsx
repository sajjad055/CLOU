...
const processingSteps: ProcessingStep[] = [
  { id: 'ckyc', labelEn: 'Pulling your details from CKYC records...', labelTa: 'CKYC பதிவுகளிலிருந்து உங்கள் விவரங்களை இழுக்கிறது...', durationMs: 2000 },
  { id: 'docs', labelEn: 'Verifying your documents...', labelTa: 'உங்கள் ஆவணங்களை சரிபார்க்கிறது...', durationMs: 1800 },
];

export function PANVerificationETBIDPage() {
  ...
  const content = {
    ...
    Tamil: {
      ...
      confirmBtn: 'OTP மூலம் உறுதிப்படுத்தவும்',
  ...
  useEffect(() => {
    ...
    if (currentProcessingStep >= processingSteps.length) {
      // CKYC pulled — review details before the existing-customer check.
      const timer = setTimeout(() => navigate('/ckyc-customer-details'), 800);
            ...
            <div className="flex flex-col items-center justify-center min-h-[65vh]">
              ...
              <div className="w-full space-y-3">
                {processingSteps.map((ps, index) => {
                  ...
                  // Only show steps that are active or already completed (progressive reveal)
          ...
          <>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                ...
                {/* CKYC info note — visually separated */}
                <div className="bg-[#eef3fa] border border-[#315C9D]/15 rounded-xl p-4">
                  <p className="text-[12px] font-semibold text-[#315C9D]">{t.ckycNote}</p>
                </div>
              </div>

              {/* Sheet CTAs */}
              <div className="px-6 py-5 border-t border-[#e5e7eb] space-y-3">
                {/* CKYC consent declaration — above primary CTA */}
                <button
                  type="button"
                  role="checkbox"
                  aria-checked={ckycConsent}
                  onClick={() => setCkycConsent(!ckycConsent)}
                  className="w-full flex items-start gap-3 text-left rounded-md"
                >
                  <span className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded-[5px] border flex items-center justify-center transition-colors ${
                    ckycConsent ? 'bg-[#315C9D] border-[#315C9D]' : 'bg-white border-[#c4c4c4]'
                  }`}>
                    {ckycConsent && <Check className="w-3.5 h-3.5 text-white" strokeWidth={3} />}
                  </span>
                  <span className="text-[12px] text-[#6b7280] leading-relaxed">{t.ckycConsent}</span>
                </button>

                <button
                  onClick={handleConfirm}
                  disabled={!ckycConsent}
                  className="w-full bg-[#315C9D] text-white h-12 rounded-lg text-base font-semibold flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed transition-opacity active:scale-[0.98]"
                >
                  {t.confirmBtn}
                  <ArrowRight className="w-5 h-5" strokeWidth={2.5} />
